...
st.markdown("""
    ...
    .stButton>button { border-radius: 0px 0px 8px 8px !important; border-top: none !important; background-color: #f1f3f5 !important; color: #0056b3 !important; font-weight: 600 !important; margin-top: -15px !important; border: 1px solid #dce1e6 !important; }
...
def get_owner_options():
    ...
    opcoes = {f"📁 {q['Name']}": q['Id'] for q in queues['records']}
    opcoes.update({f"👤 {u['Name']}": u['Id'] for u in users['records']})
...
def get_data(periodo_selecionado, dt_inicio, dt_fim, incluir_fechados):
    if periodo_selecionado == "Personalizado" and dt_inicio and dt_fim:
        ...
        mapa_periodos = {"Últimos 30 Dias": "LAST_N_DAYS:30", "Últimos 60 Dias": "LAST_N_DAYS:60", "Últimos 90 Dias": "LAST_N_DAYS:90", "Este Ano": "THIS_YEAR"}
    ...
    # QUERY: Adicionado o campo Description
    query = f"""
    SELECT 
    ...
    for record in result['records']:
        ...
        if dono_upper in filas_conhecidas:
            fila_principal, subfila = dono_upper, "-"
        elif dono_upper.startswith("CARTEIRA"):
            fila_principal, subfila = "CORPORATIVO", dono_upper
        else:
            fila_principal, subfila = "ATRIBUÍDO AO USUÁRIO", dono_upper
        ...
        classificacao = record['Account']['FOZ_Classificacao__c'] if record['Account'] and record['Account'].get('FOZ_Classificacao__c') else '-'
        ...
        linhas.append({
            ...
            'Descrição': record['Description'] # Novo Campo Visível no final
        })
...
st.markdown("""<style>[data-testid="stSidebar"] div.stButton { display: flex; justify-content: center; width: 100%; } [data-testid="stSidebar"] div.stButton > button { width: 95% !important; border-radius: 6px !important; background-color: #0056b3 !important; color: white !important; margin-top: 10px !important; border: none !important; } [data-testid="stSidebar"] div.stButton > button:hover { background-color: #004494 !important; }</style>""", unsafe_allow_html=True)
...
if periodo_selecionado == "Personalizado":
    ...
    if len(datas) == 2:
        ...
        st.sidebar.warning("Selecione as datas no calendário.")
        st.stop()
...
if "ATRIBUÍDO AO USUÁRIO" in todas_filas:
    ...
    filas_ordenadas = sorted(todas_filas) + ["ATRIBUÍDO AO USUÁRIO"]
else:
...
if df_filtrado.empty:
    ...
    for i, fila in enumerate(filas_ordenadas):
        with cols[i % 4]:
            desenhar_card(fila, df_filtrado[df_filtrado['Fila Principal'] == fila])
            st.markdown("<br>", unsafe_allow_html=True)
else:
    fila_atual = st.session_state.fila_selecionada
    st.markdown("""<style>.btn-voltar-container .stButton>button { border-radius: 6px !important; background-color: #0056b3 !important; color: white !important; margin-top: 0px !important; width: 280px !important; border: none !important; } .btn-voltar-container .stButton>button:hover { background-color: #004494 !important; }</style><div class="btn-voltar-container">""", unsafe_allow_html=True)
    ...
    if fila_atual == "CORPORATIVO":
        ...
        with col_f:
            cart_sel = st.selectbox("📌 Filtrar Carteira:", ["Todas"] + carteiras_disp)
        if cart_sel != "Todas":
    ...
    with tab1:
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 25px;">
            <div style="flex: 1; background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #0056b3;">
                <div style="font-size: 11px; color: #6c757d; font-weight: bold;">TOTAL DE CASOS</div><div style="font-size: 22px; color: #0c1c2b; font-weight: bold;">{vol}</div>
            </div>
            <div style="flex: 1; background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #f0ad4e;">
                <div style="font-size: 11px; color: #6c757d; font-weight: bold;">EM TRATATIVA</div><div style="font-size: 22px; color: #0c1c2b; font-weight: bold;">{trat}</div>
            </div>
            <div style="flex: 1; background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #00CC96;">
                <div style="font-size: 11px; color: #6c757d; font-weight: bold;">FECHADOS</div><div style="font-size: 22px; color: #0c1c2b; font-weight: bold;">{fech}</div>
            </div>
            <div style="flex: 1; background: #f8f9fa; padding: 15px; border-radius: 8px; border-left: 4px solid #6f42c1;">
                <div style="font-size: 11px; color: #6c757d; font-weight: bold;">TMA (FECHADOS)</div><div style="font-size: 22px; color: #0c1c2b; font-weight: bold;">{tma_str}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with c1:
            ...
            if not df_abertos.empty:
                ...
                fig_age = px.bar(df_abertos['Faixa'].value_counts().reindex(['0 a 3 Dias', '4 a 7 Dias', '+7 Dias']).reset_index(), x='Faixa', y='count', title='Idade dos Casos Abertos', text='count', color='Faixa', color_discrete_map={'0 a 3 Dias':'#0056b3', '4 a 7 Dias':'#f0ad4e', '+7 Dias':'#d9534f'})
        ...
        with c2:
            ...
            fig_sla = px.pie(df_sla, names='SLA Atrasado', values='count', hole=0.5, title='Saúde do SLA (Total)', color='SLA Atrasado', color_discrete_map={'✅ No Prazo':'#00CC96', '🔴 Atrasado':'#EF553B'})
    ...
    with tab2:
        ...
        with col_dl:
            ...
            st.download_button(label=f"📥 Baixar Extrato", data=to_excel(df_view), file_name=f'extrato_{fila_atual}.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        with col_aviso:
            st.caption("💡 **Dica 1:** Marque a caixa para Comentar ou Transferir em lote. **Dica 2:** Dê dois cliques no Status ou Motivo para editar a tabela.")

        def colorir_linha(row):
            return ['background-color: #ffebee' if row['SLA Atrasado'] == '🔴 Atrasado' else 'background-color: #ffffff' for _ in row]

        # Define colunas travadas. "Status" e "Motivo" ficam soltas para o usuário editar.
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Idade (Dias)": st.column_config.NumberColumn("Idade", format="%d"),
        ...
        # --- LÓGICA DE SALVAR EDIÇÕES DA TABELA ---
        ...
        # --- LÓGICA DE TRANSFERÊNCIA E COMENTÁRIOS ---
        ...
        if not casos_selecionados.empty:
            ...
            with c_transf:
                st.markdown("##### 🔄 Transferir")
                dono_selecionado = st.selectbox("Novo Proprietário:", [""] + list(lista_proprietarios.keys()))
                if st.button("Confirmar Transferência", use_container_width=True):
                    if dono_selecionado:
                        with st.spinner("Transferindo..."):
                            try:
                                payload = [{'Id': row['ID do Caso'], 'OwnerId': lista_proprietarios[dono_selecionado]} for _, row in casos_selecionados.iterrows()]
                                sf.bulk.Case.update(payload)
                                st.success("Transferidos com sucesso!")
                                st.cache_data.clear()
                                st.rerun()
                            except Exception as e:
                                st.error(f"Erro: {e}")
                    else:
                        st.warning("Selecione um proprietário.")

            with c_coment:
                st.markdown("##### 💬 Comentar em Lote")
                novo_comentario = st.text_area("Digite o comentário que será salvo em todos os casos selecionados:", height=68)
                if st.button("Inserir Comentário", use_container_width=True):
                    if novo_comentario.strip():
                        with st.spinner("Comentando..."):
                            try:
                                ...
                                st.success("Comentários inseridos!")
                                st.cache_data.clear()
                                st.rerun()
                            except Exception as e:
                                st.error(f"Erro: {e}")
